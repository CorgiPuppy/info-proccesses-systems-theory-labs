...
\begin{document}
	...
	\end{equation}
	где $\rho \frac{dS^{(l)}}{dt} = -q_{1}^{n} dFs + \int_{Fs} \frac{\mu_{1k}}{T_{1}} J_{1k}^{n} dFs$ отражает изменение энтропии смеси за счёт обмена энергией и массой с окружающей средой. \textit{1-ое слагаемое} характеризует изменение энтропии за счёт обмена энергией ($q_{1}^{n}$ - поток тепла через поверхность), \textit{2-ое слагаемое} - за счёт обмена массой с окружающей средой ($J_{1k}^{n}$ - поток массы через поверхность).
\end{document}
