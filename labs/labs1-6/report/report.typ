...
В ходе выполнения лабораторных работ было проведено исследование динамических систем, описываемых системами обыкновенных дифференциальных уравнений. Для каждой задачи были выполнены аналитические расчёты стационарных точек и определён тип их устойчивости по первому методу Ляпунова (через собственные числа матрицы Якоби). \

Затем была реализована программа на языке C++, численно решающая данные системы с использованием различных схем интегрирования (явная схема Эйлера, неявная схема, смешанные схемы). Полученные результаты визуализированы в виде фазовых портретов и графиков зависимости переменных от времени. \

Численное моделирование подтвердило аналитические выводы о типе устойчивости стационарных точек. \
- в задаче 1.1 наблюдается #underline[устойчивый узел]: траектории сходятся к стационарной точке; \
- в задаче 1.2 точка является #underline[неустойчивым узлом];  
- в задаче 1.3 наблюдается #underline[седло]: траектории приближаются к точке по одной оси и удаляются по другой;
- в задаче 1.4 система демонстрирует поведение типа #underline[центр], траектории образуют замкнутые орбиты;
- в задаче 1.5 виден #underline[устойчивый фокус]: траектории по спирали сходятся к неподвижной точке;
- в задаче 1.6 наблюдается #underline[неустойчивый фокус]: траектории по спирали расходятся от стационарной точки.
