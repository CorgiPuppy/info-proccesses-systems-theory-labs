...
На @work8-1AnalyticalSolution представлено аналитическое решение.
#figure(
	image(
		"../assets/Work8-1.jpg",
		height: 400pt,
	...
	supplement: [Рис.]
) <work8-1AnalyticalSolution>
#figure(
	image(
		"../assets/Work8-2.jpg",
		height: 400pt,
	),
	caption: [Аналитическое решение (продолжение).],
	supplement: [Рис.]
) <work8-2AnalyticalSolution>
...
Анализ по первому методу Ляпунова (в декартовых координатах) показал, что действительная часть собственных чисел матрицы линеаризации 
 меняет знак при прохождении параметра α через ноль. Это является необходимым условием возникновения бифуркации рождения предельного цикла. \
 Исследование в полярных координатах позволило установить характер устойчивости и определить радиус предельного цикла: \
При α ≤ 0 начало координат является устойчивым фокусом. Все траектории стремятся к нулю.
При α > 0 состояние равновесия в начале координат теряет устойчивость (становится неустойчивым фокусом), и вокруг него рождается устойчивый предельный цикл.
