...
int main() {
    ...
    for (long unsigned int i = 0; i < Constants::alphas.size(); i++) {
        ...
        for (Point start : starts) {
            ...
            for (double t = 0; t <= Constants::T; t += Constants::dt) {
                if (std::abs(x1) > 20.0 || std::abs(x2) > 20.0)
                    break;
    ...
    if (bifFile.is_open()) {
        for (double a = -0.5; a <= 1.0; a += 0.01) {
            if (a <= 0)
                bifFile << a << " " << 0.0 << " " << "nan" << "\n";
            else {
                double r_cycle = std::sqrt((2.0 * a) / 3.0);
                bifFile << a << " " << r_cycle << " " << 0.0 << "\n";
            }
        }
        bifFile.close();
    }
...
void step(double& x1, double& x2, double dt, double alpha) {
    ...
    Point k2 = get_derivatives(x1 + 0.5 * dt * k1.x, 
                               x2 + 0.5 * dt * k1.y, alpha);

    Point k3 = get_derivatives(x1 + 0.5 * dt * k2.x, 
                               x2 + 0.5 * dt * k2.y, alpha);

    Point k4 = get_derivatives(x1 + dt * k3.x, 
                               x2 + dt * k3.y, alpha);

    x1 += (dt / 6.0) * (k1.x + 2 * k2.x + 2 * k3.x + k4.x);
    x2 += (dt / 6.0) * (k1.y + 2 * k2.y + 2 * k3.y + k4.y);
}
