...
4. Построить фазовые портреты при следующих значениях управляющего параметра #ptgk("a"): -0.25, 0, 0.25.

Для решения использовать схему явную Эйлера:
$ cases(
	frac(x^(n+1)_1 - x^n_1, #ptgk("D") t) = f(x^(n)_1)",",
...
На @phase_minus_0_25 представлен фазовый портрет при #ptgk("a") = -0.25.
#figure(
	grid(
		image("../plots/pictures/phase_minus_0_25.png")
	),
	caption: [Фазовый портрет при #ptgk("a") = -0.25.],
	supplement: [Рис.],
) <phase_minus_0_25>
На @phase_0 представлен фазовый портрет при #ptgk("a") = 0.
#figure(
	grid(
		image("../plots/pictures/phase_0.png")
	),
	caption: [Фазовый портрет при #ptgk("a") = 0.],
	supplement: [Рис.],
) <phase_0>
На @phase_plus_0_25 представлен фазовый портрет при #ptgk("a") = 0.25.
#figure(
	grid(
		image("../plots/pictures/phase_plus_0_25.png")
	),
	caption: [Фазовый портрет при #ptgk("a") = 0.25.],
	supplement: [Рис.],
) <phase_plus_0_25>
